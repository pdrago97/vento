...
"""
    ...
    if ext in media_extensions:
        ...
        uploaded_file = await client.aio.files.upload(file=filepath)
...
@retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1, min=2, max=10))
async def chat_multimodal_ontology(filepath: str, filename: str, text: str, current_schema: dict, message: str):
    """
...
Return ONLY a JSON object in the following format:
{{
  "response": "Your conversational answer explaining what memory facts you extracted or suggest updating.",
...
If there are no new facts to add, leave the array empty.
Do not wrap your response in markdown code blocks like ```json.
"""
    ...
    try:
        ...
        try:
            ...
            print(f"Error parsing Gemini JSON response: {e}")
            return {"response": "I couldn't parse the suggestions correctly.", "suggested_memory_updates": []}
    finally:
        if uploaded_file:
            try:
                await client.aio.files.delete(name=uploaded_file.name)
...
@retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1, min=2, max=10))
async def chat_multimodal_admin(filepath: str, filename: str, text: str, current_inventory_context: dict, message: str):
    """
    Uses Gemini to analyze text or media along with a user's message, 
    providing a conversational response and suggested operational inventory updates.
    """
    if not client:
        raise Exception("GEMINI_API_KEY not configured")
        
    prompt = f"""You are an expert Operational Inventory assistant.
The user has provided a message and optionally a document or media file.
You should answer their message and suggest operational actions (like creating tickets, tasks, or updating records).

The current inventory context (if any):
...
Return ONLY a JSON object in the following format:
{{
  "response": "Your conversational answer explaining the operational insights or actions suggested.",
  "suggested_inventory_updates": [
    {{
      "type": "Task|Ticket|Document|Other",
      "action": "CREATE|UPDATE|DELETE",
      "details": {{ "title": "Example", "description": "Details here..." }}
    }}
  ]
}}

If there are no operational updates needed, leave the array empty.
Do not wrap your response in markdown code blocks like ```json.
"""
    ...
    try:
        ...
        try:
            ...
            print(f"Error parsing Gemini JSON response: {e}")
            return {"response": "I couldn't parse the suggestions correctly.", "suggested_inventory_updates": []}
    finally:
        if uploaded_file:
            try:
                await client.aio.files.delete(name=uploaded_file.name)
